...
SYSTEM_PROMPT = (
    ...
    "clearly state which platform is cheapest and by how much. For EACH item, show "
    "the specific product name and pack size that was matched (e.g. 'Milk → Amul "
    "Taaza Toned Milk 1 L, ₹69'), because the closest available match may be a "
    "different brand or the requested size may not exist — surface that so the user "
    "can judge. Point out fake discounts (where the offer price equals the MRP). "
    "Be concise; format prices in ₹. Each line item has a `status`: 'ok', "
    "'out_of_stock', or 'no_data'. NEVER call something out of stock when its "
    "status is 'no_data' — that means the data provider returned nothing for that "
    "platform (a coverage gap, not an availability fact); say 'no data available "
    "for <platform>' instead. Only say 'out of stock' for status 'out_of_stock'. "
    "Offer to find alternatives when useful.\n\n"
